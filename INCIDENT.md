...
- **2026-05-21 ~15:35** — 10 integration tests written and passing, covering all fixed scenarios
...
### Report 2 — Slow lists (diagnosed, not fixed)
...
### Not yet fixed
- **Report 2 (slow lists):** Documented as a performance improvement. Proposed fix is to push `Where`, `OrderByDescending`, and `Take` to the database, but not yet applied — downstream impacts of changing the query are unknown and need investigation first. See `TICKET.md` for follow-up.
...
- [ ] **P2:** Push list query filtering to the database — see `TICKET.md`
...
- [ ] Add index on `Tasks(UserId, CreatedAt)` for query performance at scale
